...
export default function ArchivePage() {
  ...
  if (selected) return <>
    {selected.kind === 'hackathon' ? <HackathonDetailPage item={selected} /> : <SeminarDetailPage item={selected} />}
    {isEditMode && <div className="detail-admin"><button className="admin-action" onClick={() => setEditing(selected)}>수정</button></div>}
    {editing && <ArchiveEditor value={editing} kind={editing.kind} close={() => setEditing(undefined)} />}
  </>;
  ...
  return <>
    ...
    <section className="wrap archive-section">
      <div className="section-title section-heading"><div><p className="mono cyan">HACKATHONS</p><h2>해커톤 결과물</h2></div>
        {isEditMode && <button className="admin-action" onClick={() => { setNewKind('hackathon'); setEditing(null); }}>해커톤 추가</button>}
      </div>
    ...
    {id && !selected && <p className="wrap not-found">해당 아카이브를 찾지 못했습니다.</p>}
    {editing !== undefined && <ArchiveEditor value={editing ?? undefined} kind={editing?.kind ?? newKind} close={() => setEditing(undefined)} />}
  </>;
}
