...
export default function ActivityDetailPage({ item }: { item: ActivityPost }) {
  return <main className="detail-page"><header className="detail-hero"><div className="wrap"><a className="back mono" href={sitePath('/log/')}>← 활동 기록</a><p className="mono cyan">{item.tag} · {item.cohort}기 · {displayDate(item.occurredOn)}</p><h1>{item.title}</h1><p>{item.summary}</p></div></header>
