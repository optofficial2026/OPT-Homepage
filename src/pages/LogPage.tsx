...
export default function LogPage() {
  ...
  if (selected) return <>
    <ActivityDetailPage item={selected} />
    {isEditMode && <div className="detail-admin"><button className="admin-action" onClick={() => setEditing(selected)}>수정</button></div>}
    {editing && <ActivityEditor value={editing} close={() => setEditing(undefined)} />}
  </>;

  const cohorts = [...new Set(content.activities.map((item) => item.cohort))];
  const entries = content.activities.filter((item) =>
    (filter === 'ALL' || item.tag === filter) && (cohortFilter === 'ALL' || item.cohort === cohortFilter));

  return <>
    <section className="page-head"><div className="hero-grid" /><div className="wrap">
      <a className="back mono" href={sitePath('/')}>← OPT HOME</a><p className="mono cyan">ACTIVITY LOG</p>
      <h1>활동 기록</h1><div>스터디, 세미나, 행사에서 쌓은 기록입니다.</div>
    </div></section>
    <section className="wrap section">
      <div className="section-heading"><div className="filters">{filters.map((item) =>
        <button type="button" className={filter === item ? 'is-active' : ''} key={item} onClick={() => setFilter(item)}>{item}</button>)}
      </div>{isEditMode && <button className="admin-action" type="button" onClick={() => setEditing(null)}>새 활동 기록</button>}</div>
      <div className="filters" aria-label="기수 필터"><span className="mono cyan">기수</span>
        ...
        {cohorts.map((cohort) => <button type="button" className={cohortFilter === cohort ? 'is-active' : ''} key={cohort} onClick={() => setCohortFilter(cohort)}>{cohort}기</button>)}
      </div>
    ...
    {id && !selected && <p className="wrap not-found">해당 활동 기록을 찾지 못했습니다.</p>}
    {editing !== undefined && <ActivityEditor value={editing ?? undefined} close={() => setEditing(undefined)} />}
  </>;
}
