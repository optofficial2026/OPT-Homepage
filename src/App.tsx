...
import optDarkLogo from '../opt로고 다크모드.png';
...
function Site({ page }: AppProps) {
  ...
  return <div className="min-h-screen">{current === 'home' && settings.recruitmentEnabled && <div className="ticker"><div>NOW RECRUITING　★　{settings.recruitmentCohort}기 부원 모집 중　★　STUDY · SEMINAR · HACKATHON　★　JOIN OPT　★　NOW RECRUITING　★　{settings.recruitmentCohort}기 부원 모집 중　★</div></div>}<Navigation active={current} /><PageContent /><footer><div className="wrap"><div className="brand footer-brand"><img className="footer-logo" src={optDarkLogo} alt="OPT 로고" /><span>AI Academic Club</span></div><div className="footer-admin"><span className="mono">활동 기록은 계속 업데이트됩니다.</span>{!isAdmin && <button type="button" onClick={() => setLoginOpen(true)}>관리자 로그인</button>}</div></div></footer><AdminLoginDialog /><AdminToolbar /></div>;
}
