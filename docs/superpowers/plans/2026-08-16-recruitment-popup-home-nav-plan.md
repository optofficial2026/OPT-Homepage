...
- 팝업을 닫거나 ESC·배경을 누르면 `sessionStorage`에 기록하고 같은 세션에서 다시 열지 않는다.
- 하단 `.recruit-banner`는 DOM과 미사용 전용 CSS를 제거한다.
...
- Test: `test/content-model.test.mjs`, `test/homepage.test.mjs`
...
- [ ] **Step 1: Write the failing tests**
  ...
  Add assertions that the default settings include `recruitmentPopupEnabled: true`, the repository fallback checks the new row field, the mutation writes `recruitment_popup_enabled`, and `HomeEditors` renders a `모집 팝업 표시` checkbox.
...
- [ ] **Step 3: Implement the settings contract**
  ...
  Extend the type and default object, parse the row with a `!== false` fallback, include the boolean in `updateSiteSettings`, and read the checkbox in `SettingsEditor`:
...
- [ ] **Step 4: Add the additive Supabase migration**
...
**Interfaces:**
- `RecruitmentPopup({ settings }: { settings: SiteSettings })` renders no DOM when disabled or already seen.
...
- [ ] **Step 3: Implement the minimal popup**
  ...
  In `RecruitmentPopup.tsx`, after `contentLoading` is false and both settings are true, read a fixed key such as `opt-recruitment-popup-seen` from `sessionStorage`. Render a native React overlay with `role="dialog"`, `aria-modal="true"`, a close button, recruitment copy, and the existing support URL behavior. The `close` handler must set the session key in a `try/catch` and then hide the popup; clicking the overlay closes it while clicking `.modal-box` stops propagation. Register a `keydown` effect for Escape and clean it up on unmount.

- [ ] **Step 4: Connect and style the popup**

  Expose `loading` from `useSiteContent` through `SiteContext`, render `RecruitmentPopup` from `HomePage`, remove the lower recruitment section, and add only the required `.modal`/`.modal-box` rules to `src/index.css`. Delete `.recruit-banner` rules that have no remaining caller while retaining shared `.recruit-badge` rules.
...
- [ ] **Step 3: Implement the intro page and route**
...
- [ ] **Step 4: Add minimal blank-page layout support**
...
- [ ] **Step 4: Publish and verify deployment**
